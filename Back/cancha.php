...
<head>
  ...
  <title>CDN - Modificar - Cancha</title>
  <!-- Bootstrap core CSS-->
  ...
  <div class="content-wrapper">
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Cancha</li>
      </ol>
      <div class="row">
        <div class="container">
          <div class="table-wrapper">
            ...
            $result = $mysqli->query("SELECT * FROM tblcanchas") or die($mysqli->error);
            //pre_r($result->fetch_assoc());
            ?>
            <table class="table table-striped table-hover" id="display-2">
              <div class="input-group mb-4">
                <div class="input-group-prepend">
                  <button id="button-addon7" type="submit" class="btn btn-success"><i class="fa fa-search"></i></button>
                </div>
                <input type="search" id="cancha" placeholder="¿Qué estas buscando?" aria-describedby="button-addon7" class="form-control">
                <span><a href="#addTodoModal" class="btn btn-success" data-toggle="modal">Crear Nueva Cancha</a></span>
              </div>
              <thead>
                <tr>
                  <th>Nombre</th>
              ...
              <tbody id="cancha-table">
                <?php while ($row = $result->fetch_assoc()) : ?>
                  <tr>
                    ...
                    <?php include('modals/edit_delete_modalC.php'); ?>
                  </tr>
                <?php endwhile ?>
              </tbody>
            </table>
            <button onclick="exportDataExcelCancha()" class="btn btn-success">Exportar para Excel</button>
            <button onclick="exportDataCancha()" class="btn btn-danger">Exportar para PDF</button>
            <?php
      ...
      <script>
        function exportDataCancha() {
          $('#display-2').tableExport({
            displayTableName: false,
            fileName: 'Tabla Cancha',
            ignoreColumn: [2],
            type: 'pdf',
            jspdf: {
              orientation: 'l',
        ...
        function exportDataExcelCancha() {
          $("table").tableExport({
        ...
        $(document).ready(function() {
          $("#cancha").on("keyup", function() {
            var value = $(this)
              .val()
              .toLowerCase();
            $("#cancha-table tr").filter(function() {
              $(this).toggle(
                $(this)
                .text()
                .toLowerCase()
    ...
    <div id="addTodoModal" class="modal fade">
      <div class="modal-dialog">
        <div class="modal-content">
          <form action="controller\cancha\save.php" method="post">
            <div class="modal-header">
              <h4 class="modal-title">Agregar Cancha</h4>
