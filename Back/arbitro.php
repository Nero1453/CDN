...
<head>
  ...
  <title>CDN - Modificar - Arbitros</title>
  <!-- Bootstrap core CSS-->
  ...
  <div class="content-wrapper">
    <div class="container-fluid">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Arbitros</li>
      </ol>
      <div class="row">
        <div class="container">
          <div class="table-wrapper">
            ...
            $result = $mysqli->query("SELECT * FROM tblarbitros") or die($mysqli->error);
            //pre_r($result->fetch_assoc());
            ?>
            <table class="table table-striped table-hover" id="arbitros-all">
              <div class="input-group mb-4">
                <div class="input-group-prepend">
                  <button id="button-addon7" type="submit" class="btn btn-success"><i class="fa fa-search"></i></button>
                </div>
                <input type="search" id="arbitros" placeholder="¿Qué estas buscando?" aria-describedby="button-addon7" class="form-control">
                <span><a href="#addTodoModal" class="btn btn-success" data-toggle="modal">Crear Nuevo Arbitro</a></span>
              </div>
              <thead>
                <tr>
                  <th>Nombre</th>
              ...
              <tbody id="arbitros-table">
                ...
                <tr>
                  ...
                  <?php include('modals/edit_delete_modalR.php'); ?>
                </tr>
                <?php endwhile ?>
              </tbody>
            </table>
            <button onclick="exportDataExcelArbitros()" class="btn btn-success">Exportar para Excel</button>
            <button onclick="exportDataArbitros()" class="btn btn-danger">Exportar para PDF</button>
            <?php
      ...
      <script>
        function exportDataArbitros() {
          $('#arbitros-all').tableExport({
            displayTableName: false,
            fileName: 'Tabla Arbitros',
            ignoreColumn: [2],
            type: 'pdf',
            jspdf: {
              orientation: 'l',
        ...
        function exportDataExcelArbitros() {
          $("table").tableExport({
        ...
        $(document).ready(function() {
          $("#arbitros").on("keyup", function() {
            var value = $(this)
              .val()
              .toLowerCase();
            $("#arbitros-table tr").filter(function() {
              $(this).toggle(
                $(this)
                .text()
                .toLowerCase()
    ...
    <div id="addTodoModal" class="modal fade">
      <div class="modal-dialog">
        <div class="modal-content">
          <form action="controller\referee\save.php" method="post">
            <div class="modal-header">
              <h4 class="modal-title">Agregar Arbitro</h4>
